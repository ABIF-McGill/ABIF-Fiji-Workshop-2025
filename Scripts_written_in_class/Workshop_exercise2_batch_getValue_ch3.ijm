...
if (endsWith(list_of_images[k], ".tif")) {


run("Bio-Formats Windowless Importer", "open=" + input + list_of_images[k]);
...
for (i = 0; i < numROI; i++) {
	...
	Stack.setChannel(3);
	//run("Measure");
	mean_ch3 = getValue("Mean");
	setResult("Mean_ch3", nResults-1, mean_ch3);
